...
export const MarketCard = ({ market }: { market: Market }) => {
    ...
    return (
        <div
            onClick={() => {
                router.push(`dashboard/market/${market.symbol}`);
            }}
            className="bg-white p-4 mb-2 min-w-[320px] lg:min-w-full rounded-lg shadow-sm border border-black hover:bg-[#f5f5f5]"
        >
            <div className="flex">
                <Image
                    src="/Bar_Chart.png"
            ...
            <div className="flex gap-x-4 mt-4">
                <button className="bg-blue-100 px-4 py-2 rounded-md hover:bg-blue-200 transition-colors">
                    Yes ₹{market.lastYesPrice}
                </button>
                <button className="bg-red-100 px-4 py-2 rounded-md hover:bg-red-200 transition-colors">
                    No ₹{market.lastNoPrice}
                </button>
            </div>
        </div>
    );
