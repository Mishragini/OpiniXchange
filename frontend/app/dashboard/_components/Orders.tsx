...
import {
  Order,
  useOrders,
} from "../market/[marketSymbol]/_components/OrdersProvider";
...
const OrderCard: React.FC<OrderCardProps> = ({ order, orderType }) => {
  ...
  const handleCancel = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `${endpoint}?orderId=${order.id}&marketSymbol=${order.marketSymbol}`,
        {
          method: "POST",
          credentials: "include",
          headers: {
  ...
  }, [order.id, order.marketSymbol, orderType, fetchOrders]);

  return (
    <div className="bg-white shadow-md rounded-lg p-4 my-4">
      <div className="flex justify-between items-center">
        <div className="font-medium text-gray-800">{order.side} Order</div>
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (selectedStatus !== "All") {
      orders = orders?.filter(
        (order) => order.status === selectedStatus.toUpperCase()
      );
    }
    return orders;
    ...
    <div>
      <div className="flex justify-between items-center">
        <div className="flex gap-10">
          <div
            onClick={() => setSelectOrderType("Buy")}
            className={`${
              selectOrderType === "Buy"
          ...
          <div
            onClick={() => setSelectOrderType("Sell")}
            className={`${
        ...
        <div className="flex gap-5">
          <div
            ...
            className={`${
              selectedStatus === "All" ? "text-black" : "text-gray-500"
            } cursor-pointer`}
          >
            All
          </div>
          <div
            ...
            className={`${
              selectedStatus === "Pending" ? "text-black" : "text-gray-500"
            } cursor-pointer`}
          >
            Pending
          </div>
          <div
            onClick={() => setSelectedStatus("Partially_Filled")}
            className={`${
              selectedStatus === "Partially_Filled"
                ? "text-black"
                : "text-gray-500"
            } cursor-pointer`}
          >
          ...
          <div
            ...
            className={`${
              selectedStatus === "Filled" ? "text-black" : "text-gray-500"
            } cursor-pointer`}
          >
            Filled
          </div>
          <div
            ...
            className={`${
              selectedStatus === "Cancelled" ? "text-black" : "text-gray-500"
            } cursor-pointer`}
          >
            Cancelled
          </div>
      ...
      <div className="my-2">
        {(!filteredOrders || filteredOrders.length === 0) && <Empty />}
        {filteredOrders?.map((order) => (
          <OrderCard key={order.id} order={order} orderType={selectOrderType} />
        ))}
      </div>
    </div>
  );
